...
private:
  ...
  struct SampleBuffer {
    std::vector<std::vector<PixelColorStorage> > sub_pixels; // @suppress("Type cannot be resolved")
    ...
    void fill_color(int i, int j, Color c) {
      ...
      unsigned char r = (unsigned char) round(c[0]*255);
      unsigned char g = (unsigned char) round(c[1]*255);
      unsigned char b = (unsigned char) round(c[2]*255);
      vector<unsigned char> vect{r,g,b};
      p = vect;
      return;
    }
    ...
    Color get_pixel_color() {
      ...
      return Color(rsum/total, gsum/total, bsum/total);
