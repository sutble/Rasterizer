...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  // Part 6: Pass in correct barycentric differentials to tri->color for mipmapping.
	...
	float T0[] = {x1-x0,y1-y0};
	float T1[] = {x2-x1,y2-y1};
	float T2[] = {x0-x2,y0-y2};

	float N0[] = {-T0[1],T0[0]};
	float N1[] = {-T1[1],T1[0]};
	float N2[] = {-T2[1],T2[0]};

	int xmin = (int) min(x0,min(x1,x2))-1;
	int xmax = (int) max(x0,max(x1,x2))+1;
	int ymin = (int) min(y0,min(y1,y2))-1;
	int ymax = (int) max(y0,max(y1,y2))+1;

	for( float x = xmin; x < xmax; x++){
		for (float y = ymin; y < ymax; y++){
			int den = 2 * sqrt(sample_rate);
			for (int numx = 1; numx < den; numx = numx+2){
				for (int numy = 1; numy < den; numy = numy+2){
					float cx = x + ((float) numx)/((float) den);
					float cy = y + ((float) numy)/((float) den);

					float V0[] = {cx-x0,cy-y0};
					float V1[] = {cx-x1,cy-y1};
					float V2[] = {cx-x2,cy-y2};

					float L0 = V0[0]*N0[0] + V0[1]*N0[1]; //Dot product
					float L1 = V1[0]*N1[0] + V1[1]*N1[1]; //Dot product
					float L2 = V2[0]*N2[0] + V2[1]*N2[1]; //Dot product

					float La = -T2[0]*N0[0] - T2[1]*N0[1];
					float Lb = -T0[0]*N1[0] - T0[0]*N1[0];
					float Lc = -T1[0]*N2[0] - T1[1]*N2[0];
					Vector3D p_bary(0.0,0.0,0.0);

					if (tri != NULL){
							//need to calculate barycentric coordinates
						//p_bary[0] = L0/La;
						//p_bary[1] = L1/Lb;
						//p_bary[2] = 1 - p_bary[0] - p_bary[1];

						p_bary[0] = (-(cx - x1)*(y2-y1) + (cy - y1)*(x2-x1))/(-(x0-x1)*(y2-y1)+(y0-y1)*(x2-x1));
						p_bary[1] = (-(cx - x2)*(y0-y2) + (cy - y2)*(x0-x2))/(-(x1-x2)*(y0-y2)+(y1-y2)*(x0-x2));
						p_bary[2] = 1 - p_bary[0] - p_bary[1];
					...
					if (L0 >= 0 && L1 >= 0 && L2 >= 0){
						...
						//std::cout << p_bary[0] << p_bary[1] << p_bary[2] << " \n";

						int sub_row = (numy + 1)/2 -1;
						int sub_column = (numx + 1)/2 -1;
